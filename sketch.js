...
class obj{ //宣告一個類別，一個物件/圖案就是一個類別，針對所畫的圖，設定的基本條件
    constructor(args){//預設值，物件的基本資料（ex.物件的顏色、移動的速度、物件的大小、物件初始顯示位置）
      ...
      this.v = createVector( random(-10,1), random(-10,1))//物件移動的速度，x軸正負為左右，y軸正負為上下
...
function setup() {
  ...
  for(var i = 0 ; i < 3; i = i + 1){//設定迴圈，if i+1，i=0~9共繞回圈10次；if i+2，i=0,2,4,6,8
...
function draw() {//每秒執行60次
  ...
  for (let ball of balls){//針對陣列變數，取出陣列內一個一個的物件
    ...
    for (let monster of monsters){
      if(ball.isBallInRanger(monster.p.x,monster.p.y)){
        score=score-1
       // elephant_sound.play()
        //balls.splice(balls.indexOf(ball),1)
  ...
  for (let monster of monsters){//針對飛彈韓庫內資料
    ...
    for (let bullet of bullets){
      if(monster.isBallInRanger(bullet.p.x,bullet.p.y)){
        ...
        //elephant_sound.play()
        //monsters.splice(monsters.indexOf(monster),1)
        //monster.IsDead=true//設定為true就是被打到了，接下來執行爆炸的畫面
